...
def main():
    ...
    for key in SECRET_KEYS:
        ...
        values[key] = (env.get(env_key) or "").strip()

    # Automatically read the GCP JSON file content if GOOGLE_APPLICATION_CREDENTIALS is set in .env
    gcp_path_str = env.get("GOOGLE_APPLICATION_CREDENTIALS", "")
    if gcp_path_str and not values.get("GCP_SERVICE_ACCOUNT_KEY"):
        gcp_path = Path(gcp_path_str)
        if gcp_path.exists():
            values["GCP_SERVICE_ACCOUNT_KEY"] = gcp_path.read_text().strip()
